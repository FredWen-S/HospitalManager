<template>
    <div>
        ...
        <el-card>
            ...
            ></el-alert>
            <el-alert
                v-if="sectionLoaded && sectionData.length === 0"
                title="当前科室暂无可预约医生，请返回科室列表重新选择或前往全科门诊咨询。"
                type="warning"
                show-icon
                :closable="false"
                class="guidance-alert"
...
export default {
    ...
    data() {
        return {
            ...
            //拼接时间和日期成为oId
            idTime: "",
            fromGuidance: this.$route.query.fromGuidance === "1",
            guidanceDepartment: this.$route.query.guidanceDepartment || "",
            sectionLoaded: false,
        };
    },
    computed: {
        guidanceAlertText() {
            if (
                this.guidanceDepartment &&
                this.guidanceDepartment !== this.sectionOpt
            ) {
                return (
                    "智能导诊建议科室：" +
                    this.guidanceDepartment +
        ...
        dateClick(date) {
            ...
            request
                ...
                .then((res) => {
                     ...
                     * 重点！！！把数组中的对象取出来用map
                     */
                    const arrangeList = res.data.data || [];
                    this.sectionData = arrangeList.map((item) => item.doctor);
                    this.clickTag = true;
                });
        },
        //获取当天及后7天的日期星期
